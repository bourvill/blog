...
Après 2 ans de pratique de KMM/KMP, j'avais envie de faire un debrief de cette énièmes chimères multiplateformes.
...
Nous avons du trouver et organiser l’équipe afin de commencer à collaborer ensemble autour d’un code commun.
...
Nos approches sont totalement différentes, nous aimons les computed properties pour des valeurs simples, coté android écrire des methods. Il faut donc trouver un premier juste milieu et gérer la frustration de chacun sur la façon de faire. Ceci est un exemple, mais voilà le genre d'arbitrage à faire, et quand ce n'est pas votre langage de prédilection, vous êtes très vite à court d'argument sur la façon de faire.

Cet argument reste bien entendu un élèment propre aux membres de votre équipe, mais il est à prendre en compte si vous voulez migrer vers KMM car vous allez faire travailler deux équipes sur du code commun et qui auront probablement deux mindset different.
...
Le problème est que quand vous avez un lot d'extension swift afin de simplifier votre code sur la gestion des Int, String, Bool par exemple. Vous vous retrouver à devoir mapper des extensions afin de faire passe plat entre votre code et le code KMM afin de garder vos mécaniques déjà en place.
...
Les enum n'existe pas. Il faut passer par des companion ([https://kotlinlang.org/docs/object-declarations.html#companion-objects](https://kotlinlang.org/docs/object-declarations.html#companion-objects "companion")) pour arriver à certains résultat.

Votre enum CaseIterable? Nop vous devez écrire un companion qui vous map vos case. Un comportement different de langage qui vous ampute de fonctionnalité clé de votre langage favoris.

Vient donc le choix d'installer une dépendance à [SKIE](https://skie.touchlab.co/ "https://skie.touchlab.co/"). Un outil permettant d'avoir enfin un mapping des Kotlin sealed class vers un enum swift. Il permet et facilite aussi la gestion d'autres soucis, la doc explique plutot bien les mapping que SKIE apporte.

On met donc en place une jambe de bois afin de palier au manque du premier framework. On commence donc à mettre des dépendances à d'autres projet, suivis de version, bug, monté de version... KMM évoluant assez vite, SKIE le doit donc aussi.
...
Nous avons également essayé via les StateResult et là, c'est le drame, puisque nous sommes de notre côté sur iOS obligés de downcast les objets
...
Je n'arrive pas à comprendre cette obsession que les autres développeur ont de vouloir absolument foutre un mouton à 5 pattes sur iOS pour ne simplement ne pas faire du natif, quand on voit maintenant les capacités de SwiftUI. Pourquoi vouloir absolument se rendre dépendant d'un framework/environnement tiers quant finalement Apple passe son temps à optimiser et améliorer l'outil qu'il vous donne...
